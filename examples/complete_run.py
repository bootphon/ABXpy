...
def dtw_cosine_distance(pt_1, pt_2):
    """Return the DTW cosine distance between two points.
    """
...
def prepare_working_dir(working_dir='./data'):
    ...
    files = dict((k, os.path.join(working_dir, v))
                 for (k, v) in files.iteritems())

    # Remove existing files in data directory
    for abx_file in files.values():
        if os.path.isfile(abx_file):
            os.remove(abx_file)
