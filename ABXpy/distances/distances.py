...
def run_distance_job(job_description, distance_file, distance,
    ...
    if not(splitted_features):
        ...
        for feature_file, feature_group in zip(feature_files, feature_groups):
            t, f = h5features.read(feature_file, feature_group)
            assert(not(set(times.keys()).intersection(t.keys())),
                   "The same file is indexed by (at least) two different "
                   "feature files")
            times.update(t)
            features.update(f)
        get_features = Features_Accessor(times, features).get_features_from_raw
    ...
    for b in range(n_blocks):
        ...
        for i in range(n_pairs):
            ...
            try:
                ...
                raise(ValueError("Error with the files {0} and {1}: {2}"
...
if __name__ == '__main__':
    ...
    # parser (the usage string is specified explicitly because the default does
    # not show that the mandatory arguments must come before the mandatory ones
    # otherwise parsing is not possible beacause optional arguments can have
    # various numbers of inputs)
    parser = argparse.ArgumentParser(
        usage="%(prog)s features task [distance] [-o output]",
        description='ABX distance computation')
    # I/O files
    g1 = parser.add_argument_group('I/O files')
    g1.add_argument('features', help='feature file')
    ...
    g1.add_argument(
        '-n', '--ncpu', default=None, help='optional: number of cpus to use')
    args = parser.parse_args()
    compute_distances(args.features, '/features/', args.task, args.output,
                      dtw_cosine_distance, n_cpu=args.ncpu)
