...
# custom read_table that ignore empty entries at the end of a file (they can result from trailing white spaces at the end for example)
def read_table(filename):
	db = pandas.read_table(filename, sep='[ \t]+')
	db = db.dropna(how='all') # removes row with all null values (None or NaN...)
	return db
...
def load(filename, features_info=False):
    ...
    # finding '#' (to separate location info from attribute info) and fixing names of columns
    ...
    assert len(l) > 0 and l[0] == 0, 'The first column name in the database main file must be prefixed with # (sharp)'
    assert len(l) == 2, 'Exactly two column names in the database main file must be prefixed with a # (sharp)'
    feat_db = db[db.columns[:l[1]]]    
    db = db[db.columns[l[1]:]]
    # verbose print("  Read input File '"+filename+"'. Defined conditions: "+str(newcolumns[attrI:len(columns)]))
    
    # opening up existing auxiliary files, and merging with the main database
    # and creating a forest describing the hierarchy at the same time (useful for optimizing regressor generation and filtering)
    ...
    feat_db = feat_db[~ nanrows]
    	# not so verbose print('** Warning ** ' + len(nanrows) + ' items were removed because of missing information. The removed items are listed in' + basename + '-removed.item' 
    if features_info:
    ...
    for i, col in enumerate(cols):
        ...
        try:
            ...
            if not(auxfile == mainfile):
                ...
                assert col == auxdb.columns[0], 'First column name in file %s is %s. It should be %s instead.' % (auxfile, auxdb.columns[0], col)
                # call get_aux_dbs on child columns
                auxdb, auxforest = load_aux_dbs(basename, auxdb, auxdb.columns[1:])
                # add to forest
                forest[i].addChildrenFromList(auxforest)
                # merging the databases
                db = pandas.merge(db, auxdb, on=col, how='left')
        # verbose print("  Read auxiliary File '"+auxfile+"'. Defined conditions: "+str(newcol[1:len(newcol)])+" on key '"+newcol[0]+"'")
        except IOError:
            pass
    return db, forest