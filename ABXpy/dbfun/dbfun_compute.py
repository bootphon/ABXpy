...
class nameVisitor(ast.NodeVisitor):  
    ...
    def visit_Name(self, node):
        self.names.append(node.id)# -*- coding: utf-8 -*-
"""
Created on Mon Oct 14 16:59:27 2013
...
class DBfun_Compute(dbfun.DBfun):
    ...
    def parse(self):
        ...
        tree = ast.parse(self.script)
        # find and extract imports
        imports = [stat for stat in tree.body
                   if isinstance(stat, (ast.ImportFrom, ast.Import))]
        rest = [stat for stat in tree.body
                if not(isinstance(stat, (ast.ImportFrom, ast.Import)))]
        # store ast with import statements
        # can be executed later using: exec(self.import_bytecode)
        self.import_bytecode = compile(ast.Module(imports), '', mode='exec')
        tree = ast.Module(rest)
        ...
        expression = tree.body[-1]
        if not(isinstance(expression, ast.Expr)):
            raise ValueError('The following script should finish by an '
                             'expression: %s' % self.script)
        # store what is left
        self.main_ast = tree
        """
    ...
    def process_with(self, tree):
        ...
        for i, w in enumerate(withs):
            if isinstance(w.context_expr, ast.Str):
                kept.append((i, w))
        if len(kept) > 1:
            raise ValueError('There is more than one with statement for '
                             're-using auxiliary ABX files in script: %s'
                             % self.script)
        if len(kept) == 1:
            # find the h5 files and aliases
            s = kept[0][1]
