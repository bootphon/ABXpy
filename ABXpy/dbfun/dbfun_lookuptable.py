...
# compatible with unicode, plus what about pandas ? in particular
# isinstance(s, str) is problematic since it evaluates to false for unicode
# strings...
# should be replaced by isinstance(s, (str, unicode))
class DBfun_LookupTable(dbfun.DBfun):
        ...
        with h5py.File(self.filename) as f:
            ...
            if 'code' in f:
                ...
                if not(self.input_names == self.computer.input_names):
                    raise ValueError(
                        'Some input columns defined in the synopsis of file %s'
                        ' are unused in the corresponding script %s' % (
    ...
    def get_keys(self, input_values):
        ...
        if isinstance(input_values, collections.Mapping):
            ...
            indexes = [[self.indexes[i_type].index(
                e) for e in input_values[i]]
                for i, i_type in enumerate(self.input_types)]
        # this is vectorial
