...
class FilterManager(side_operations_manager.SideOperationsManager):
    ...
    def __init__(self, db_hierarchy, on, across, by, filters):
        ...
        self.generic = [] # this case is specific to filters, it applies a generic filter to the database before considering A, B and X stuff.
  
        # associate each of the provided filters to the appropriate point in the computation flow
        for filt in filters: # filt can be: the name of a column of the database (possibly extended), the name of lookup file, the name of a script, a script under the form of a string (that doesnt end by .dbfun...)
            ...
            if filt in self.extended_cols: # column already in db
                ...
                # evaluate context is wasteful in this case... not even necessary to have a dbfun at all                                
            elif len(filt) >= 6 and filt[-6:] == '.dbfun': # lookup table
                db_fun = dbfun_lookuptable.DBfun_LookupTable(filt, indexed=False) # ask for re-interpreted indexed outputs
    ...
    def classify_generic(self, elements, db_fun, db_variables):
        # check if there are only non-extended names and, only if this is the case, instantiate 'generic' field of db_variables
    ...
    def by_filter(self, by_values): return singleton_filter(self.evaluate_by(by_values))       
    
    def generic_filter(self, by_values, db): return db.iloc[vectorial_filter(lambda context: self.evaluate_generic(by_values, db, context), np.arange(len(db)))]
...
def vectorial_filter(generator, indices):
    ...
    for result in generator(context):
        ...
        kept = kept[still_up]
        for var in context:
            context[var] = [context[var][e] for e in still_up] # keep testing only the case that are still possibly True
