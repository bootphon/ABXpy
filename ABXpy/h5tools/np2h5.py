...
class NP2H5(object):
    ...
    def __init__(self, h5file): # sink is the name of the HDF5 file to which to write, buffer size is in kilobytes                  
        ...
        try:
            if self.file_open:
                ...
                if self.manage_file:
                    self.file.close()
                    self.file_open = False
            # if there was an error, delete dataset, otherwise check that the amount of data actually written is consistent with the size of the datasets
            if eValue is not None:
                ...
                if self.manage_file:
                    self.file.close()
                    self.file_open = False
            # check that all buffers were completed (defaults to true for dataset without a fixed size)  
            elif not(all([buf.iscomplete() for buf in self.buffers])):               
                raise Warning('File %s, the amount of data actually written is not consistent with the size of the datasets' % self.filename)
        except:
            # raise the first exception
            if eValue is not None:
                #FIXME the first exception will be raised, but could log a warning here ...
                pass
            else:
                raise
    ...
    def add_dataset(self, group, dataset, n_rows=0, n_columns=None, chunk_size=100, buf_size=100, item_type=np.int64, overwrite=False, fixed_size=True):        
        if n_columns is None:
            raise ValueError('You have to specify the number of columns of the dataset.')
        if self.file_open:
            buf = NP2H5buffer(self, group, dataset, n_rows, n_columns, chunk_size, buf_size, item_type, overwrite, fixed_size)        
            self.buffers.append(buf)
            return buf 
        else:
            raise IOError("Method add_dataset of class NP2H5 can only be used within a 'with' statement!")  
...
class NP2H5buffer(object):
    ...
    def __init__(self, parent, group, dataset, n_rows, n_columns, chunk_size, buf_size, item_type, overwrite, fixed_size):
        ...
        if n_rows==0 and fixed_size:
            raise ValueError('A dataset with a fixed size cannot have zero lines')
        if overwrite and not(fixed_size):
            raise ValueError('Cannot overwrite a dataset without a specified fixed size')
        self.fixed_size = fixed_size
                
        # check type argument
        self.type = np.dtype(item_type) #  dtype call is needed to access the itemsize attribute in case a built-in type was specified        
        if self.type.itemsize == 0:
            raise AttributeError('NP2H5 can only be used with numpy arrays whose items have a fixed size in memory')
        ...
        self.dataset_ix = 0
        if group + '/' + dataset in parent.file: # fail if dataset already exists and overwrite=False otherwise create it or overwrite it    
            if overwrite:
                ...
                if self.dataset.shape[0] != n_rows or self.dataset.shape[1] != n_columns or self.dataset.dtype != self.type:
                    raise IOError('Overwriting a dataset is only possible if it already has the correct shape and dtype')
            else:
                raise IOError('Dataset %s already exists in file %s!' % (dataset, parent.filename))
        else:
            ...
            # create dataset  
            if self.fixed_size:
                g.create_dataset(dataset, (n_rows, n_columns), dtype=self.type) # would it be useful to chunk here?
            else:
                chunk_lines = nb_lines(self.type.itemsize, n_columns, chunk_size)
                g.create_dataset(dataset, (n_rows, n_columns), dtype=self.type, chunks=(chunk_lines, n_columns), maxshape=(None, n_columns))               
            self.dataset = parent.file[group][dataset]
    ...
    def write(self, data):
        ...
        if not(self.parent.file_open):
            raise IOError("Method write of class NP2H5buffer can only be used within a 'with' statement of parent NP2H5 object!")
        target_ix = self.buf_ix + data.shape[0]
        # if size is not of fixed size, check that it is big enough
        if not(self.fixed_size):
            necessary_rows = self.dataset_ix + self.buf_len*(target_ix//self.buf_len)
            if necessary_rows > self.n_rows:
                self.n_rows = necessary_rows
                self.dataset.resize((self.n_rows, self.n_columns)) # maybe should use larger increments ? could use chunk size as a basis for the increments instead of buf_len if useful              
