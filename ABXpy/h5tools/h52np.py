...
class H52NP(object):
        ...
        if self.file_open and self.manage_file:
            try:
            	...
            	if eValue is not None:
                	# the first exception will be raised, but could log a warning here ...
                	pass
            	else:
                	raise
    ...
    def add_dataset(self, group, dataset, buf_size=100, minimum_occupied_portion=0.25):        
        if self.file_open:
            buf = H52NPbuffer(self, group, dataset, buf_size, minimum_occupied_portion)        
            self.buffers.append(buf)
            return buf 
        else:
            raise IOError("Method add_dataset of class H52NP can only be used within a 'with' statement!") 
...
class H52NPbuffer(object):
    ...
    def __init__(self, parent, group, dataset, buf_size, minimum_occupied_portion):
        ...
        if not(group + '/' + dataset in parent.file): # fail if dataset do not exist    
        ...
        # initialize buffer  
        row_size = self.n_columns * self.type.itemsize / 1000. # entry size in kilobytes         
        self.buf_len = int(round(buf_size/row_size))
        self.buf_ix = self.buf_len # buf_ix represents the number of free rows in the buffer. Here the buffer is empty
        self.buf = np.zeros((self.buf_len, self.n_columns), self.type)
        # fill it
        self.refill_buffer()

    
    # read and consume, refill automatically if the buffer becomes empty, if there is not enough data left, just send less than what was asked   
    def read(self, amount=None):
        
        assert self.parent.file_open
        
        if not(amount is None) and amount <= 0:
            raise ValueError('The amount to be read in h52np.read must be strictly positive')
        ...
        while amount_found < amount and not(self.isempty()):
            ...
            # fill buffer or not, according to refill policy and current buffer state
            self.refill_buffer() 
    ...
    def refill_buffer(self):
        if not(self.dataset_ix == self.n_rows):
            # for now one policy is implemented: if less than self.minimum_occupied_portion of the full capacity is occupied the buffer is refilled        
