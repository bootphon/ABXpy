...
import numpy as np
import tempfile, os, h5py, np2h5, h52np #FIXME shutil
...
class H5Handler(object):
    ...
    def __init__(self, h5file, keygroup, keyset, groups=None, datasets=None):
        ...
        if keypath in paths:
            raise ValueError('key dataset %s should not be in the list of datasets to be sorted: %s' % (keypath, paths))
        self.file = h5file
            ...
            with H5TMP() as tmp:        
                with h5py.File(self.file) as f:       
                    ...
                    for g, d in self.sources:
                        if not(g + '/' + d in f): 
                            raise IOError('Dataset %s does not exist in group %s of file %s!' % (d, g, self.file))
    
                    # set tmp file structure (cannot be done in __init__ because the tmp file is destroyed as soon as the 'with' statement is exited)
                    ...
                    if not(all([n_row == n_rows[0] for n_row in n_rows])):
                        raise IOError('First dimensions of all the datasets should be the same')
                    self.n_row = n_rows[0]    
                    self.n_columns = [dset.shape[1] for dset in dsets]
                    self.dtypes = [dset.dtype for dset in dsets]
                    dset_weights = [dset.shape[1]*dset.dtype.itemsize for dset in dsets]
                    self.dsets = dsets
                    self.line_weight = np.sum(dset_weights)                          
                    buf_rows = int(round(buffer_size*1000./self.line_weight))
                    row = 0
                    n_chunks = 0
                ...
                try:
                    with np2h5.NP2H5(self.file) as o:
                        with h52np.H52NP(tmp) as i:
                            ...
                            i_buf_rows = int(round(buf_rows/n_chunks)) # number of rows
                            assert i_buf_rows > 0   # might need to set up an explicit case for dealing with files that can fit in one chunk and remove these asserts                     
                            i_buf = []
                            for ix, (g, d) in enumerate(self.sources):
                                buf = []
                                for c in range(n_chunks):
                                    buf_size = self.dtypes[ix].itemsize*self.n_columns[ix]*i_buf_rows/1000.
                                    buf.append(i.add_dataset(g, d + '_' + str(c), buf_size))
                                i_buf.append(buf)
                            
                            # create n_datasets output buffers       
                            o_buf_rows = int(round(1000.*o_buffer_size/self.line_weight))
                            assert o_buf_rows > 0
                            o_buf = []
                            for ix, (g, d) in enumerate(self.sources):
                                buf_size = self.dtypes[ix].itemsize*self.n_columns[ix]*o_buf_rows/1000.
                                o_buf.append(o.add_dataset(g, d, self.n_row, self.n_columns[ix], buf_size, buf_size, self.dtypes[ix], overwrite=True))                   
                                
                            # some redundancy could be removed from the following while + flushing    
                            # while not all input datasets empty:   
                            while not(all([buf.dataset_empty() for buf in i_buf[0]])):  # sufficient to check key buffers only

                                # find min of maxes (current tails) in key
                                min_tail = min([buf.current_tail() for buf in i_buf[0] if not(buf.isempty())])      
                                
                                # find for each chunk the amount to be read
                                amounts = [buf.nb_lower_than(min_tail) for buf in i_buf[0]]
                                print(min_tail)
                                print(amounts)
                                
                                # get the order on key for the concatenated data
                                data = []
                                # for all chunks: read up to the appropriate index, concatenate, sort and output
                                for c in range(n_chunks):
                                    if amounts[c] > 0:
                                        data.append(i_buf[0][c].read(amounts[c]))
                                data = np.concatenate(data)
                                order = np.argsort(data[:,0])
                                o_buf[0].write(data[order,:])
                                
                                # for all other dset use the order from the key:
                                for d in range(1, len(self.datasets)):
                                    data = []
                                    # for all chunks: read up to the appropriate index, concatenate, sort and output
                                    for c in range(n_chunks):
                                        if amounts[c] > 0:
                                            data.append(i_buf[d][c].read(amounts[c]))
                                    data = np.concatenate(data)
                            ...
                            for c in range(n_chunks):
                                if not(i_buf[0][c].buffer_empty()):
                                    data.append(i_buf[0][c].read()) # read without argument gives the whole buffer
                            ...
                            for d in range(1, len(self.datasets)):
                                ...
                                for c in range(n_chunks):
                                    if not(i_buf[d][c].buffer_empty()):
                                        data.append(i_buf[d][c].read()) # read without argument gives the whole buffer
                                if data: # False iff list is empty
                                    data = np.concatenate(data)
                                    o_buf[d].write(data[order,:]) 
                ...
                # use the backup                
                    ...
                    # shutil.copyfile(self.backup, self.file) #FIXME not usable if backup isn't made                        
                    raise
        # delete the backup
        finally:
    ...
    def extract_chunk(self, i_start, i_end, chunk_id):
        ...
        for i, d in enumerate(self.dsets):
            ...
            g = self.groups[i]
            self.tmp[g].create_dataset(chunk_name, (i_end-i_start, self.n_columns[i]), dtype=d.dtype)
            self.tmp[g][chunk_name][:,:] = chunk
            
            
           
# Class for creating a temporary h5 file. Should be used with the 'with' statement. The temporary file will be erased upon exiting the 'with' statement
class H5TMP(object):
...
def test():
    ...
    # test sort on it
    h = H5Handler(os.path.join(folder, 'tmp.h5'), 'key', 'k', ['data', 'data'], ['f', 'i'])
    h.sort(100, 100)
