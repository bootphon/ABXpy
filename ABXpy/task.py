...
class Task(object):
    ...
    def __init__(self, db_name, on, across=None, by=None, filters=None,
                 regressors=None, verbose=0, verify=True):
    ...
    def compute_statistics(self, approximate=False):
        ...
        if self.verbose > 0:
            display = progress_display.ProgressDisplay()
            display.add(
                'block', 'Computing statistics for by/on/across block', self.stats['nb_blocks'])
        ...
        for by, db in self.by_dbs.iteritems():
            ...
            for block_key, count in stats['on_across_levels'].iteritems():
                ...
                on, across = on_across_from_key(
                    block_key)  # retrieve the on and across keys (as they are stored in the panda object)
                ...
                if self.filters.on_across_by_filter(on_across_by_values):
                    ...
                    if (approximate or not(self.filters.A or self.filters.B or self.filters.X or self.filters.ABX)) and type(across) != tuple:
                        ...
                        triplets = self.on_across_triplets(
                            by, on, across, block, on_across_by_values, with_regressors=False)
    ...
    def on_across_triplets(self, by, on, across, on_across_block,
        ...
        if self.across == ['#across']:
            ...
            B = np.array(
                list(set(self.by_dbs[by].index).difference(on_set)), dtype=self.types[by])
        else:
    ...
    def generate_triplets(self, output=None, sample=None):
        ...
        if self.verbose > 0:
            display = progress_display.ProgressDisplay()
            display.add(
                'block', 'Computing triplets for by/on/across block', self.n_blocks)
            display.add(
        ...
        for by, db in self.by_dbs.iteritems():
            ...
            with np2h5.NP2H5(h5file=output) as fh:
                ...
                with h5io.H5IO(filename=output, datasets=datasets, indexes=indexes, group='/regressors/' + str(by) + '/') as out_regs:
                    if sample is not None:
                        n_rows = np.uint64(
                            round(sample * (self.by_stats[by]['nb_triplets'] / np.float(self.total_n_triplets))))
                    else:
                    ...
                    out = fh.add_dataset(group='triplets', dataset=str(
                        by), n_rows=n_rows, n_columns=3, item_type=self.types[by], fixed_size=False)
                    ...
                    for block_key, block in self.on_across_blocks[by].groups.iteritems():
                        ...
                        if self.filters.on_across_by_filter(on_across_by_values):
                            ...
                            if self.verbose > 0:
                                ...
                                display.update(
                                    'triplets', self.by_stats[by]['block_sizes'][block_key])
                        if self.verbose > 0:
                            display.display()
            ...
            if not_empty:
                ...
                with h52np.H52NP(output) as f_in:
                    with np2h5.NP2H5(output) as f_out:
                        ...
                        out = f_out.add_dataset(
                            'pairs', str(by), n_columns=1, item_type=pair_key_type, fixed_size=False)
                ...
                with h52np.H52NP(output) as f_in:
                    with np2h5.NP2H5(output) as f_out:
                        ...
                        out = f_out.add_dataset(
                            'unique_pairs', str(by), n_columns=1, item_type=pair_key_type, fixed_size=False)
                        try:
                            last = -1
                            while True:
                                pairs = inp.read()
                ...
                store.append('/feat_dbs/' + str(by), self.feat_dbs[by], expectedrows=len(
                    self.feat_dbs[by]))  # use append to make use of table format, which is better at handling strings without much space (fixed-size format)
                store.close()
    ...
    def compute_nb_levels(self):
        if self.filters.A or self.filters.B or self.filters.X or self.filters.ABX:
            raise ValueError(
                'Current implementation do not support computing nb_levels in th presence of A, B, X, or ABX filters')
        if self.verbose > 0:
            display = progress_display.ProgressDisplay()
            display.add(
                'block', 'Computing nb_levels for by block', self.stats['nb_by_levels'])
        for by, db in self.by_dbs.iteritems():
            if self.verbose > 0:
                display.update('block', 1)
                display.display()
            ...
            for block_key, n_block in self.by_stats[by]['on_across_levels'].iteritems():
                ...
                if self.filters.on_across_by_filter(on_across_by_values):
                    ...
                    if self.across == ['#across']:
                        ...
                        B = np.array(
                            list(set(self.by_dbs[by].index).difference(X)), dtype=self.types[by])
                    else:
                        B = self.across_blocks[by].groups[across]
                        # remove B with the same 'on' than A
                        B = np.array(
...
if __name__ == '__main__':
    ...
    g1.add_argument(
        'database', help='main file of the database defining the items used to form ABX triplets and their attributes')
    g1.add_argument('output', nargs='?', default=None,
                    help='optional: output file, where the results of the analysis will be put')
    # Task specification
    g2 = parser.add_argument_group('Task specification')
    g2.add_argument(
        '-o', '--on', required=True, help='ON attribute, ' + message)
    g2.add_argument('-a', '--across',  nargs='+',
                    default=[], help='optional: ACROSS attribute(s), ' + message)
    g2.add_argument('-b', '--by', nargs='+',
                    default=[], help='optional: BY attribute(s), ' + message)
    g2.add_argument('-f', '--filt', nargs='+',
                    default=[], help='optional: filter specification(s), ' + message)
    g2.add_argument('-s', '--sample', default=None, type=float,
                    help='optional: if a real number in ]0;1[: sampling proportion, if a strictly positive integer: number of triplets to be sampled')
    # Regressors specification
    g3 = parser.add_argument_group('Regressors specification')
    g3.add_argument('-r', '--reg', nargs='+',
                    default=[], help='optional: regressor specification(s), ' + message)
