...
Toy example of usage:
    sampler = IncrementalSampler(10**4, 10**4, step=100, relative_indexing=\
False)
    complete_sample = np.concatenate([sample for sample in sampler])
    assert all(complete_sample==range(10**4))
...
More realistic example of usage: sampling without replacement 1 million items
from a total of 1 trillion items, considering 100 millions items at a time
    sampler = IncrementalSampler(10**12, 10**6, step=10**8, relative_indexing=\
False)
    complete_sample = np.concatenate([sample for sample in sampler])
"""
...
# function np.random.hypergeometric is buggy so I did my own implementation...
# (error, at least, line 784 in computation of variance: sample used instead of
# m, but this can't be all of it ?)
...
# (https://github.com/numpy/numpy/blob/master/numpy/random/mtrand/distributions
    #.c and see original ref in zotero)
...
def hypergeometric_sample(N, K, n):
    ...
    if N == 0 or N == 1:
        ...
        # using symmetries to speed up computations
        K_eff = min(K, N-K)  # if the probability of failure is smaller than
        # the probability of success, draw the failure count
        n_eff = min(n, N-n)  # if the amount of items to sample from is larger
        # than the amount of items that will remain, draw from the items that
        # will remain
        N_float = np.float64(N)  # useful to avoid unexpected roundings
        ...
        p_mode = (math.lgamma(mode+1) + math.lgamma(K_eff-mode+1) +
                  math.lgamma(n_eff-mode+1)+math.lgamma(N-K_eff-n_eff+mode+1))
        upper_bound = min(min(n_eff, K_eff)+1,
                          np.floor(a+16*np.sqrt(variance+0.5)))
        ...
        while True:
            ...
            if k < 0 or k >= upper_bound:
                ...
                p_k = (math.lgamma(k+1) + math.lgamma(K_eff-k+1) +
                       math.lgamma(n_eff-k+1) + math.lgamma(N-K_eff-n_eff+k+1))
                d = p_mode-p_k
                if U*(4-U)-3 <= d:
                    break
                if U*(U-d) >= 1:
